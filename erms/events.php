...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'register') {
    ...
    if (!$event_id) {
        $msg = 'Invalid event.'; $msg_type = 'error';
    } else {
        ...
        if (!$ev) {
            $msg = 'Event not found or no longer available.'; $msg_type = 'error';
        } else {
            // Check for ANY row (including cancelled) — uq_user_event covers all statuses
            $exists = $pdo->prepare("SELECT registration_id, status FROM registrations WHERE user_id = ? AND event_id = ?");
            $exists->execute([$user_id, $event_id]);
            $existing = $exists->fetch(PDO::FETCH_ASSOC);

            if ($existing && $existing['status'] !== 'cancelled') {
                // Already actively registered
                $msg = 'You are already registered for this event.'; $msg_type = 'error';
            } else {
                ...
                if ((int)$enrolled->fetchColumn() >= $ev['max_slots']) {
                    $msg = 'Sorry, this event is already full.'; $msg_type = 'error';
                } else {
                    try {
                        if ($existing && $existing['status'] === 'cancelled') {
                            // Previously cancelled — UPDATE instead of INSERT to avoid uq_user_event violation
                            $pdo->prepare("UPDATE registrations SET status = 'confirmed', registered_at = NOW() WHERE registration_id = ?")
                        ...
                        if ($e->getCode() === '23000') {
                            ...
                            $msg = 'Something went wrong. Please try again.'; $msg_type = 'error';
                        }
                    }
                }
            }
...
<aside class="sidebar" id="sidebar">
  <div class="sidebar-brand">
    <div class="sb-brand-top">
      ...
      <div class="sb-brand-text"><h1>ERMS</h1><p>Student Portal</p></div>
    </div>
    <div class="sb-clock">
...
<main class="main">
  <div class="page">
    ...
    <div class="counts-row">
      <div class="count-chip blue"><div class="dot"></div><?= (int)$counts['total'] ?> Total</div>
      <div class="count-chip green"><div class="dot"></div><?= (int)$counts['registered'] ?> Registered</div>
      <div class="count-chip gold"><div class="dot"></div><?= (int)$counts['available'] ?> Available</div>
    </div>
...
.sidebar-brand h1{ font-family:var(--ff-d); font-size:.93rem; font-weight:600; color:var(--text); line-height:1.25; }
.sidebar-brand p { font-size:.63rem; color:var(--text-3); letter-spacing:.09em; text-transform:uppercase; margin-top:1px; }
...
.sb-clock__date{ font-size:.61rem; color:var(--text-3); letter-spacing:.05em; margin-top:4px; text-transform:uppercase; }
...
function openDetail(ev) {
  ...
  if (ev.description) { document.getElementById('detail-desc').textContent = ev.description; descRow.style.display='flex'; }
...
(function() {
  ...
  function tick() {
    ...
    var tEl=document.getElementById('sbTime'), dEl=document.getElementById('sbDate');
