    ...
    <div class="topbar">
        ...
        <div style="position:relative;display:inline-block;margin-right:8px;" id="exportDropWrap">
            ...
            <div id="exportDrop" style="display:none;position:absolute;top:calc(100% + 6px);right:0;min-width:220px;background:var(--bg2,#141824);border:1px solid var(--bdr);border-radius:10px;box-shadow:0 8px 28px rgba(0,0,0,.35);z-index:999;overflow:hidden;">
                ...
                <div style="padding:8px 6px;">
                    ...
                    <button onclick="doExport('registrations','csv')" class="exp-item"><span>&#128196;</span> CSV <span style="font-size:.7rem;color:var(--t3);">(.csv)</span></button>
                    <button onclick="doExport('registrations','excel')" class="exp-item"><span>&#128202;</span> Excel <span style="font-size:.7rem;color:var(--t3);">(.xls)</span></button>
                    <button onclick="doExport('registrations','pdf')" class="exp-item"><span>&#128424;</span> PDF <span style="font-size:.7rem;color:var(--t3);">(print / save)</span></button>
                </div>
            </div>
        </div>
        <style>
        ...
        <script>
            ...
            function doExport(type, format) {
                ...
                var base = "q=" + encodeURIComponent(document.querySelector('[name=q]')?.value || '') + "&status=" + encodeURIComponent(document.querySelector('[name=status]')?.value || 'all') + "&event_id=" + encodeURIComponent(document.querySelector('[name=event_id]')?.value || '0');
                        ...
                        <div class="rdd rdd--wide" id="rdd-event">
                            ...
                            <button type="button" class="rdd__btn" onclick="rddToggle('rdd-event')">
                                <span id="rdd-event-lbl"><?php
                                                            ...
                                                            else foreach ($events_list as $ev) if ($ev['event_id'] === $event_f) {
                                                                echo htmlspecialchars(mb_strimwidth($ev['title'], 0, 28, '…'));
                                                                break;
                                                            }
                                                            ?></span>
                                <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
