...
(function () {
  ...
  var bar   = document.getElementById('strengthBar') || document.getElementById('strengthFill');
  ...
  pw.addEventListener('input', function () {
    ...
    if (len === 0) {
      ...
      bar.style.background = '#c45c5c';
      label.textContent    = 'Too short — must be at least 8 characters';
      label.style.color    = '#c45c5c';
    } else {
      bar.style.width      = '100%';
      bar.style.background = '#4e9b72';
      label.textContent    = 'Good ✓ (8+ characters)';
      label.style.color    = '#4e9b72';
    }
...
(function () {
  var pw      = document.getElementById('password');
  var confirm = document.getElementById('confirm_password');
  var hint    = document.getElementById('confirmHint') || document.getElementById('matchHint');
...
/* ── 7. PASSWORD SHOW/HIDE ───────────────────────────────────
   ...
   ----------------------------------------------------------- */
(function () {
  var map = { pwToggle: 'password', pwToggle1: 'password', pwToggle2: 'confirm_password' };
...
(function () {
  ...
  input.addEventListener('input', function () {
    ...
    if (!v) { hint.textContent = '7-digit number, e.g. 3240000'; hint.className = 'input-hint'; return; }
    if (/^\d{7}$/.test(v)) {
      hint.textContent = '✓ Valid format'; hint.className = 'input-hint valid';
      input.classList.remove('is-error'); input.classList.add('is-ok');
    } else {
      hint.textContent = 'Must be exactly 7 digits'; hint.className = 'input-hint invalid';
...
/* ── 9. SUBMIT LOADING STATE ─────────────────────────────────
   ...
   ----------------------------------------------------------- */
(function () {
  var pairs = [['registerForm', 'Creating account…'], ['regForm', 'Creating account…']];
...
(function () {
  ...
  function animateCounter(el) {
    ...
    (function tick(now) {
      ...
      el.childNodes[0].textContent = Math.round(ease(progress) * target).toLocaleString();
